...
void lv_port_disp_init(void)
{
    ...
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "gpio_config(backlight) failed: %s", esp_err_to_name(ret));
        return;
    }
...
#if ((LCD_5r0_800x480==1)||(LCD_4r3_800x480==1)||(LCD_7r0_800x480==1))
    ...
    ret = esp_lcd_rgb_panel_register_event_callbacks(panel_handle, &cbs, &disp_drv);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_lcd_rgb_panel_register_event_callbacks failed: %s", esp_err_to_name(ret));
        return;
    }
    ...
    ret = esp_lcd_rgb_panel_get_frame_buffer(panel_handle, 2, &buf1, &buf2);
    if (ret != ESP_OK) {
        ESP_LOGE(TAG, "esp_lcd_rgb_panel_get_frame_buffer failed: %s", esp_err_to_name(ret));
        return;
    }
    // initialize LVGL draw buffers
